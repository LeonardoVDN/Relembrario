...
<template>
  <div>
    <!-- Alertas de Sucesso e Erro (fora do modal) -->
  ...
  methods: {
    ...
    async fetchTags() {
      ...
      try {
        ...
        this.$emit("memory-edit-error", "Não foi possível carregar as tags.");
      } finally {
    ...
    async submitEdit() {
      ...
      try {
        ...
        this.$emit("memory-edited", "Memória editada com sucesso!");
        this.closeModal();
      } catch (error) {
        console.error("Erro ao editar memória:", error);
        this.$emit("memory-edit-error", error.message);
      }
    },
    resetForm() {
